...
export const categoriesApi = {
  async list(): Promise<Paged<ProduceCategory>> {
    const { data } = await api.get<ApiEnvelope<Paged<ProduceCategory>>>('/marketplace/categories/')
    return unwrap(data)
  },
}
...
export const listingsApi = {
  ...
  async create(payload: ListingPayload): Promise<Listing> {
    const { data } = await api.post<ApiEnvelope<Listing>>('/marketplace/listings/', payload)
    return unwrap(data)
  },
  async update(id: number, payload: Partial<ListingPayload>): Promise<Listing> {
    const { data } = await api.patch<ApiEnvelope<Listing>>(`/marketplace/listings/${id}/`, payload)
    return unwrap(data)
  },
