...
export const harvestsApi = {
  ...
  async update(id: number, payload: Partial<HarvestPayload>): Promise<Harvest> {
    const { data } = await api.patch<ApiEnvelope<Harvest>>(`/finance/harvests/${id}/`, payload)
    return unwrap(data)
  },
  async remove(id: number): Promise<void> {
...
export const salesApi = {
  ...
  async update(id: number, payload: Partial<SalePayload>): Promise<Sale> {
    const { data } = await api.patch<ApiEnvelope<Sale>>(`/finance/sales/${id}/`, payload)
    return unwrap(data)
  },
  async remove(id: number): Promise<void> {
...
export const profitLossApi = {
  async get(filters: ProfitLossFilters = {}): Promise<ProfitLoss> {
    const params = Object.fromEntries(Object.entries(filters).filter(([, v]) => v))
    const { data } = await api.get<ApiEnvelope<ProfitLoss>>('/finance/profit-loss/', { params })
    return unwrap(data)
  },
}