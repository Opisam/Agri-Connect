...
import type { Notification, Paged } from './types'

export const notificationsApi = {
  async list(params?: { is_read?: boolean }): Promise<Paged<Notification>> {
    const { data } = await api.get<ApiEnvelope<Paged<Notification>>>('/notifications/', {
      params,
    })
    return unwrap(data)
  },
  ...
  async markRead(id: number): Promise<Notification> {
    const { data } = await api.patch<ApiEnvelope<Notification>>(`/notifications/${id}/`, {
      is_read: true,
    })
    return unwrap(data)
  },
